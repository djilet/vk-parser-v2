...
export type UploadableVkMessage = {
  id: number;
  date: number;
  out: 0 | 1;
  from_id: number;
  text?: string | null;
  attachments?: unknown[];
  reply_message?: UploadableVkMessage;
  fwd_messages?: UploadableVkMessage[];
  action?: unknown;
};

export type MessageUploadRow = {
 ...
 */
export function mapMessageToUploadRow(message: UploadableVkMessage): MessageUploadRow | null {
  if (message.action) {
    return null;
  }
