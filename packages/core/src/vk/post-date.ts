...
function resolveMonthName(monthName: string): number | undefined {
...
function buildDateWithOptionalYear(
  referenceDate: Date,
  day: number,
  month: number,
  year: number | null,
...
/** Parses VK's Russian relative/absolute post-date text ("только что", "5 мин назад", "вчера в 12:30", "5 мая 2026") into an ISO timestamp with offset. */
export function parseVkPostDateText(text: string | null | undefined, referenceDate = new Date()): string | null {
  if (!text) {
    return null;
  }
  ...
  match = normalized.match(/^(\d{1,2})\s+([а-яё]+)\.?(?:\s+(\d{4}))?(?:\s+в\s+(\d{1,2}):(\d{2}))?$/);
  if (match) {
    ...
    return formatTimestamp(buildDateWithOptionalYear(now, day, month, year, hours, minutes));
  }

  return null;
}

export function pickLatestPostDate(dateTexts: Array<string | null | undefined>, referenceDate = new Date()): string | null {
