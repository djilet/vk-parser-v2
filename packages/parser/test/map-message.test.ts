...
import { mapMessageToUploadRow } from '../src/vk/map-message.js';
...
test('mapMessageToUploadRow: out=0 is is_my_message=false', () => {
  const row = mapMessageToUploadRow({ id: 1, date: 1, out: 0, from_id: 1, text: 'hi', attachments: [] });
...
test('mapMessageToUploadRow drops messages with no text and no attachments', () => {
  const row = mapMessageToUploadRow({ id: 1, date: 1, out: 0, from_id: 1, text: '', attachments: [] });
    ...
    attachments: [
      {
        ...
        photo: {
          sizes: [
            { type: 'm', width: 130, url: 'https://example.com/small.jpg' },
            { type: 'x', width: 604, url: 'https://example.com/big.jpg' },
          ],
        },
      },
    ],
...
test('mapMessageToUploadRow expands a forwarded-only message instead of dropping it', () => {
  const row = mapMessageToUploadRow({
    ...
    from_id: 1,
    text: '',
    fwd_messages: [{ id: 0, date: 0, out: 0, from_id: 0, text: 'смотри, вот их прайс', attachments: [] }],
  });
