import type { VkConversationItem, VkGroup, VkProfile } from '@vk-sales-bot/core';
...
export function resolvePeerPhotoUrl(
  ...
  if (peer.type === 'user') {
    const profile = profiles.find((entry) => entry.id === peer.id);
    return pickPhoto100(profile as (VkProfile & { photo_100?: string }) | undefined);
  }

  if (peer.type === 'group') {
    const group = groups.find((entry) => entry.id === Math.abs(peer.id));
    return pickPhoto100(group as (VkGroup & { photo_100?: string }) | undefined);
  }
