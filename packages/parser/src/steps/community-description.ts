...
export type CommunityDescriptionReason = 'ok' | 'empty' | 'no_button' | 'no_modal';
export type CommunityDescriptionResult = { description: string | null; reason: CommunityDescriptionReason };
 ...
 *
 * Возвращает { description, reason }, где reason различает пустое описание ('empty' — модалка
 * открылась, но текста нет) и неответившую страницу ('no_button' / 'no_modal'). По второму
 * варианту вызывающий код понимает, что VK начал блокировать парсер.
 */
export async function parseCommunityDescription(page: Page): Promise<CommunityDescriptionResult> {
  try {
  ...
  try {
    const { status, description } = await page.evaluate(
      (modalSel, statusSel, descSel) => {
        ...
        const readCell = (selector: string) => {
          const cell = modalElement.querySelector(selector);
          const content = cell?.querySelector('.vkuiMiniInfoCell__middle')?.textContent ?? cell?.textContent ?? null;
          return cleanText(content);
        };

        return { status: readCell(statusSel), description: readCell(descSel) };
      },
      MODAL,
      STATUS_CELL,
      DESC_CELL,
