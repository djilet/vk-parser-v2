import { parsePeerIdFromMsgUrl, pickLatestPostDate, sleep } from '@vk-sales-bot/core';
import type { Page } from 'puppeteer';
import { parseCommunityDescription } from './community-description.js';
...
function printCommunityInfo(data: ScrapedCommunity): void {
...
export async function parseCommunityPage(page: Page): Promise<ScrapedCommunity> {
  console.log(`Жду загрузку страницы сообщества (${PAGE_LOAD_WAIT_MS / 1000}с)...`);
  await sleep(PAGE_LOAD_WAIT_MS);

  const data = await page.evaluate(() => {
    const cleanText = (value: string | null | undefined) => value?.replace(/\s+/g, ' ').trim() || null;

    const nameElement = document.querySelector('[data-testid="group-name"]');
    const name = cleanText(nameElement?.querySelector('span')?.textContent ?? nameElement?.textContent ?? null);
    ...
    if (phoneElement) {
      ...
      phone = cleanText(
        phoneLink?.textContent ?? phoneLink?.getAttribute('href')?.replace(/^tel:/, '') ?? phoneElement.textContent,
      );
    }
    ...
    if (siteElement) {
      const siteLink = siteElement.querySelector('a[href]') as HTMLAnchorElement | null;
    ...
    const isProfileHref = (href: string | null): boolean => {
      ...
      const excludedPrefixes = [
        '/settings', '/support', '/feed', '/friends', '/groups', '/search', '/audio', '/video',
        '/market', '/clips', '/games', '/apps', '/mail', '/notifications', '/bookmarks', '/club',
        '/public', '/event',
      ];

      return (
        !excludedPrefixes.some((prefix) => path === prefix || path.startsWith(`${prefix}/`)) && path.startsWith('/')
      );
    };

    const isInSidebar = (element: Element): boolean =>
      !!element.closest('#side_bar, [id="side_bar"], [data-testid="side_bar"], [class*="LeftMenu"], [class*="left_menu"]');

    const getContactsContainer = (): Element | null => {
      const infoAnchor = document.querySelector('[data-testid="group-info-phone"], [data-testid="group-info-site"]');
      ...
      return (
        groupName?.closest('[class*="TwoColumnLayoutNarrow"]') ?? groupName?.closest('[class*="TwoColumnLayout"]') ?? null
      );
    };

    const parseContactCell = (cell: Element) => {
      ...
      if (subtitleElement) {
        const subtitleClone = subtitleElement.cloneNode(true) as Element;
      ...
      const email = cleanText(emailLink?.textContent ?? emailLink?.getAttribute('href')?.replace(/^mailto:/, '') ?? null);
