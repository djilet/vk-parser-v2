import type { VkConversation, VkMessage } from '@vk-sales-bot/core';
import type { ExportedFile, ExportedMessage } from '@vk-sales-bot/contracts';

export type { ExportedFile, ExportedMessage };
  ...
  for (const attachment of attachments) {
    ...
    switch (item.type) {
      ...
      case 'sticker': {
        ...
        const images = Array.isArray(sticker?.images) ? (sticker.images as { url?: string; width?: number }[]) : [];
...
export function formatMessages(messages: VkMessage[]): ExportedMessage[] {
  return [...messages].sort((a, b) => a.date - b.date || a.id - b.id).map(formatMessage);
}
