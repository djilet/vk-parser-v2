...
import { BROWSER_IDS, type BrowserId } from '../accounts.js';
import { getBrowserPaths, repoPath } from '../paths.js';
...
function buildSavedToken(
  ...
  return {
    ...
    ...token,
    expiresAt:
      token.expiresAt ?? (token.expiresIn ? new Date(Date.now() + token.expiresIn * 1000).toISOString() : undefined),
    savedAt,
  };
}
...
/** Legacy single-account token files from before the two-browser split — migrated once, on read. */
async function migrateLegacyToken(browserId: BrowserId): Promise<SavedToken | null> {
  const legacyPaths = [repoPath(`.vk-token-${browserId}.json`), repoPath('.vk-token.json')];
...
export async function saveToken(token: SavedToken): Promise<SavedToken> {
  ...
  // 0o600 — this file holds a live VK access token; keep it unreadable by other local users.
...
export async function getAccessToken(browserId: BrowserId): Promise<string> {
  ...
  if (!token) {
    throw new Error(`Токен для браузера #${browserId} не найден. Запустите: vk-sales-bot vk token --browser ${browserId}`);
  }

  if (isTokenExpired(token)) {
    throw new Error(`Токен для браузера #${browserId} истёк. Запустите: vk-sales-bot vk token --browser ${browserId}`);
  }
...
export async function getTokenByUserId(userId: number): Promise<SavedToken> {
  ...
  if (!token) {
    ...
    throw new Error(
      `Токен для accountId=${userId} не найден.${available ? ` Доступные: ${available}` : ' Сначала получите токен: vk-sales-bot vk token'}`,
    );
  }

  if (isTokenExpired(token)) {
    throw new Error(
      `Токен для accountId=${userId} истёк. Запустите: vk-sales-bot vk token --browser ${token.browserId}`,
    );
  }
...
export async function saveTokenFromUrl(url: string, browserId: BrowserId, appId?: number): Promise<SavedToken> {
  const parsed = extractTokenFromUrl(url);
  if (!parsed) {
    throw new Error('В URL нет access_token');
  }

  return saveTokenForBrowser(browserId, { ...parsed, appId });
}
