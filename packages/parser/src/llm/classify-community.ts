import { completeChat, type YandexEnv } from './yandex-gpt.js';
...
export type ClassifiableCommunity = {
  id: number;
  name?: string | null;
...
export type ClassificationResult = { isOrganizer: boolean; raw: string };

function buildCommunityBlock(community: ClassifiableCommunity): string {
...
function buildUserPrompt(communities: ClassifiableCommunity[]): string {
  return communities.map(buildCommunityBlock).join('\n\n');
}

function parseAnswer(word: string): boolean | null {
...
/**
 * Разбирает ответ LLM построчно, привязывая каждый результат к id сообщества — так один
 * сбитый или потерянный пункт не портит остальные результаты батча.
 */
function parseBatchAnswer(raw: string, requestedIds: Set<number>): Map<number, ClassificationResult> {
...
/**
 * Спрашивает у LLM за один запрос про несколько сообществ сразу — резко сокращает число
 * вызовов Yandex GPT (и повторную пересылку системного промпта на каждое сообщество).
 * `results` — только по id, чей ответ удалось разобрать; `rawResponse` пригодится для отладки.
 */
export async function classifyCommunities(
  env: YandexEnv,
  communities: ClassifiableCommunity[],
): Promise<{ results: Map<number, ClassificationResult>; rawResponse: string }> {
